...
class Lista{
    ...
    public void mergesort() {
     ...
     public void intercalar(int esq, int meio, int dir){
        ...
        sentinela.setUpvotes(0x7FFFFFFF);
        ...
        for(i = j = 0, k = esq; k <= dir; k++){
           //array[k] = (a1[i].getUpvotes() <= a2[j].getUpvotes()) ? a1[i++].clone() : a2[j++].clone();
           if (a1[i].getUpvotes() < a2[j].getUpvotes()){
            array[k] = a1[i++].clone();
           }
           else if (a1[i].getUpvotes() > a2[j].getUpvotes()){
            array[k] = a2[j++].clone();
           }
           else{
            if (a1[i].getName().compareTo(a2[j].getName()) < 0) array[k] = a1[i++].clone();
            else array[k] = a2[j++].clone(); 
           }
        }
     }
}
...
class TP03Q09{
    ...
    public static void main(String[] args) throws Exception {
        ...
        File log = new File("matricula_mergesort.txt");
