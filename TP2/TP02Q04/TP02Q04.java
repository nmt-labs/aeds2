...
class Lista {
    ...
    //no need to create a function to insert on the start cause we're only gonna insert at the end
    ...
    public void inserirFim(Game x) throws Exception {
       ...
       array[n] = x.clone(); //clone the game into the array element
       //array[n].imprimir(); //pra conferir se inseriu
       n++;
    }
    ...
    public Game remover(int pos) throws Exception {
       ...
       //move the elements to the previous posision
       for(int i = pos; i < n; i++){
    ...
    public void exibir(){
        for(int i = 0; i < n; i++) MyIO.println(array[i].getName());
    }
